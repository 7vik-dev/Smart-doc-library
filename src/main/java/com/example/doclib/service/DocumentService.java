...
public class DocumentService {
    ...
    /**
     * Returns all documents, using cache to avoid repeated DB queries.
     *
     * Teaching Note - How @Cacheable works:
     *
     * FIRST CALL (Cache MISS):
     *   1. Spring checks cache named "documents"
     *   2. Cache is empty → MISS
     *   3. Method body runs → queries MongoDB
     *   4. Result stored in cache
     *   5. Result returned to caller
     *
     * SECOND CALL (Cache HIT):
     *   1. Spring checks cache named "documents"
     *   2. Cache has data → HIT!
     *   3. Method body is SKIPPED ENTIRELY (never runs)
     *   4. Cached result returned directly
     *
     * You can verify this by watching the logs:
     * Cache MISS → you'll see "Fetching from MongoDB"
     * Cache HIT  → you'll see only "Cache HIT" (no MongoDB log)
     */
    @Cacheable("documents")
    public List<Document> getAllDocuments() {
        totalRequests++;

        // This log appears ONLY on a cache miss (method body is skipped on hit)
        log.info("📦 [CACHE MISS] getAllDocuments - querying MongoDB (request #{})", totalRequests);
        cacheMisses++;
        ...
        log.info("   ✅ MongoDB returned {} documents in {}ms", docs.size(), duration);
        log.info("   💾 Result is now stored in cache 'documents'");
        log.info("   ⚡ Next call will be served from memory (no DB query)");
    ...
    /**
     * Uploads a document - extracts content and saves to MongoDB.
     ...
     * getAllDocuments() to re-query MongoDB and get fresh results.
     */
    ...
    public Document createDocument(String title, String content) {
        ...
        log.info("   🗑️  Cache cleared - next list request hits MongoDB");
    ...
    public List<Document> searchDocuments(String title) {
        totalRequests++;
        log.info("🔍 [SEARCH CACHE MISS] Searching MongoDB for title containing: '{}'", title);
        cacheMisses++;
    ...
    public Map<String, String> requestSummary(String documentId) {
        ...
        if ("PROCESSING".equals(doc.getSummaryStatus())) {
    ...
    public Map<String, Object> getStats() {
        ...
        long completedSummaries = documentRepository.countBySummaryStatus("COMPLETED");
        long processingSummaries = documentRepository.countBySummaryStatus("PROCESSING");
